<template>
        ...
        <div class="flex items-center justify-between h-14">
          ...
          <div v-if="displayedTrip" class="hidden sm:flex items-center">
            <TripCountdown
              ...
              :trip-status="displayedTrip.status"
              :compact="true"
            />
          </div>
          ...
          <div class="flex items-center gap-2 flex-shrink-0">
            ...
            >
              ...
              <span class="hidden sm:inline">Despachar Viaje</span>
            </button>
      ...
      <div class="px-3 sm:px-4 lg:px-6 mt-6">
        <div class="max-w-screen-2xl mx-auto">
          <TripPackagesSection
            ...
            @receive-package="handleReceivePackage"
          />
        </div>
      </div>
    </div>
  ...
  <!-- Modal de registro de encomiendas -->
  ...
  <!-- Modal de recepción de encomienda -->
  <PackageReceptionModal
    :show-modal="showReceptionModal"
    :package-data="selectedPackageForReception"
    @close="closeReceptionModal"
    @confirm="onReceivePackageConfirm"
  />

  <!-- Modal de confirmación para Terminar Viaje -->
  <teleport to="body">
    <div v-if="showFinishTripModal" class="fixed inset-0 z-[60] overflow-y-auto" aria-labelledby="modal-title" role="dialog" aria-modal="true">
      <div class="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
        <div class="fixed inset-0 bg-gray-500 bg-opacity-75 transition-opacity" aria-hidden="true" @click="showFinishTripModal = false"></div>
        ...
        <div class="inline-block align-bottom bg-white rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full">
          <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
            <div class="sm:flex sm:items-start">
              ...
              <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
                <h3 class="text-lg leading-6 font-medium text-gray-900" id="modal-title">Terminar Viaje</h3>
                <div class="mt-2">
                  <p class="text-sm text-gray-500">
                    ¿Estás seguro de que deseas dar por terminado este viaje? <br><br>
                    Al hacerlo, podrás marcar las encomiendas que estaban en tránsito como "En Destino" para poder entregarlas a los clientes.
                  </p>
                </div>
              </div>
            </div>
          ...
          <div class="bg-gray-50 px-4 py-3 sm:px-6 sm:flex sm:flex-row-reverse">
            <button type="button" class="w-full inline-flex justify-center rounded-md border border-transparent shadow-sm px-4 py-2 bg-green-600 text-base font-medium text-white hover:bg-green-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-green-500 sm:ml-3 sm:w-auto sm:text-sm disabled:opacity-50" @click="executeFinishTrip" :disabled="finishingTrip">
              {{ finishingTrip ? 'Terminando...' : 'Sí, terminar viaje' }}
            </button>
            <button type="button" class="mt-3 w-full inline-flex justify-center rounded-md border border-gray-300 shadow-sm px-4 py-2 bg-white text-base font-medium text-gray-700 hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 sm:mt-0 sm:ml-3 sm:w-auto sm:text-sm" @click="showFinishTripModal = false" :disabled="finishingTrip">
              Cancelar
            </button>
          </div>
        </div>
  ...
  <!-- Modal de confirmación para Despachar Viaje -->
  <teleport to="body">
    <div v-if="showDispatchTripModal" class="fixed inset-0 z-[60] overflow-y-auto" aria-labelledby="modal-title" role="dialog" aria-modal="true">
      <div class="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
        <div class="fixed inset-0 bg-gray-500 bg-opacity-75 transition-opacity" aria-hidden="true" @click="showDispatchTripModal = false"></div>
        <span class="hidden sm:inline-block sm:align-middle sm:h-screen" aria-hidden="true">&#8203;</span>
        <div class="inline-block align-bottom bg-white rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full">
          <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
            <div class="sm:flex sm:items-start">
              ...
              <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
                <h3 class="text-lg leading-6 font-medium text-gray-900" id="modal-title">Despachar Viaje</h3>
                <div class="mt-2">
                  <p class="text-sm text-gray-500">
                    ¿Estás seguro de que deseas despachar este viaje? <br><br>
                    Al hacerlo, todas las encomiendas asignadas pasarán automáticamente a estado "En tránsito".
                  </p>
                </div>
              </div>
            </div>
          ...
          <div class="bg-gray-50 px-4 py-3 sm:px-6 sm:flex sm:flex-row-reverse">
            <button type="button" class="w-full inline-flex justify-center rounded-md border border-transparent shadow-sm px-4 py-2 bg-blue-600 text-base font-medium text-white hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 sm:ml-3 sm:w-auto sm:text-sm disabled:opacity-50" @click="executeDispatchTrip" :disabled="dispatchingTrip">
              {{ dispatchingTrip ? 'Despachando...' : 'Sí, despachar' }}
            </button>
            <button type="button" class="mt-3 w-full inline-flex justify-center rounded-md border border-gray-300 shadow-sm px-4 py-2 bg-white text-base font-medium text-gray-700 hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 sm:mt-0 sm:ml-3 sm:w-auto sm:text-sm" @click="showDispatchTripModal = false" :disabled="dispatchingTrip">
              Cancelar
            </button>
          </div>
        </div>
...
// Funciones para finalizar viaje
const showFinishTripModal = ref(false)
const finishingTrip = ref(false)

const promptFinishTrip = () => {
  showFinishTripModal.value = true
}

const executeFinishTrip = async () => {
  finishingTrip.value = true
  try {
    await tripStore.finishTrip(tripId.value)
    showNotification('success', 'Viaje terminado', 'El viaje ha sido marcado como "Terminado". Las encomiendas han sido puestas en destino.')
    showFinishTripModal.value = false
    await refreshTripSilently()
    await fetchTripPackages(tripId.value)
  } catch (error) {
    console.error('Error terminando viaje:', error)
    showNotification('error', 'Error', error.response?.data?.detail || 'Ocurrió un error al intentar finalizar el viaje.')
  } finally {
    finishingTrip.value = false
  }
}
...
const handleDeliverPackage = (packageId) => {
  const pkg = tripPackages.value.find(p => p.id === packageId)
  if (pkg) {
...
const selectedPackageForReception = ref(null)

const handleReceivePackage = (packageId) => {
  const pkg = tripPackages.value.find(p => p.id === packageId)
  if (pkg) {
    selectedPackageForReception.value = pkg
...
const closeReceptionModal = () => {
  ...
  selectedPackageForReception.value = null
}

const onReceivePackageConfirm = async (packageId) => {
  try {
    const config = useRuntimeConfig()
    await $fetch(`${config.public.apiBaseUrl}/packages/${packageId}/update-status`, {
      method: 'PUT',
      body: { new_status: 'arrived_at_destination' }
    })
    closeReceptionModal()
    showNotification('success', 'Paquete recibido', 'El paquete ha sido registrado en la oficina de destino.')
    if (displayedTrip.value?.id) {
      await fetchTripPackages(displayedTrip.value.id)
    }
  } catch (error) {
    console.error('Error al recibir paquete:', error)
    showNotification('error', 'Error', error.response?.data?.detail || 'Error al actualizar el estado del paquete.')
  }
}

// Funciones para despachar viaje
const showDispatchTripModal = ref(false)
const dispatchingTrip = ref(false)

const promptDispatchTrip = () => {
  showDispatchTripModal.value = true
}

const executeDispatchTrip = async () => {
  dispatchingTrip.value = true
  try {
    await tripStore.dispatchTrip(tripId.value)
    showNotification('success', 'Viaje despachado', 'El viaje ha sido despachado y las encomiendas están en tránsito.')
    showDispatchTripModal.value = false
    await refreshTripSilently()
    await fetchTripPackages(tripId.value)
  } catch (error) {
    console.error('Error despachando viaje:', error)
    showNotification('error', 'Error', error.response?.data?.detail || error.message || 'Ocurrió un error al intentar despachar el viaje.')
  } finally {
    dispatchingTrip.value = false
  }
}
