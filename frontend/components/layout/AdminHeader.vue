<template>
  <header class="bg-white shadow-sm w-full relative z-40">
        ...
        <nav class="hidden md:flex space-x-4">
          <ClientOnly>
            ...
            <template v-if="authStore.userRole === 'secretary'">
              ...
              <NuxtLink to="/trips" class="text-gray-600 hover:text-gray-900 px-3 py-2 rounded-md text-sm font-medium" active-class="text-indigo-600 bg-indigo-50">Viajes</NuxtLink>
              ...
              <NuxtLink to="/packages" class="text-gray-600 hover:text-gray-900 px-3 py-2 rounded-md text-sm font-medium" active-class="text-indigo-600 bg-indigo-50">Encomiendas</NuxtLink>
          ...
          <div class="ml-3 relative">
            <div>
              <button @click="isUserMenuOpen = !isUserMenuOpen" type="button" class="bg-white rounded-full flex text-sm focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500" id="user-menu-button" aria-expanded="false" aria-haspopup="true">
                ...
                <div v-if="userInfo?.profileImageUrl" class="h-8 w-8 rounded-full overflow-hidden">
                  <img class="h-full w-full object-cover" :src="userInfo.profileImageUrl" alt="User profile">
                </div>
                <div v-else class="h-8 w-8 rounded-full bg-gradient-to-r from-blue-600 to-indigo-600 flex items-center justify-center text-white font-bold text-sm shadow-sm border border-white">
                  {{ userInitials }}
                </div>
              </button>
            </div>
            ...
            >
              <div ref="userMenu" v-if="isUserMenuOpen" class="origin-top-right absolute right-0 mt-2 w-48 rounded-md shadow-lg py-1 bg-white ring-1 ring-black ring-opacity-5 focus:outline-none z-50" role="menu" aria-orientation="vertical" aria-labelledby="user-menu-button" tabindex="-1">
                ...
                <a href="#" @click.prevent="handleLogout" class="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-100" role="menuitem" tabindex="-1" id="user-menu-item-1">Cerrar sesión</a>
              </div>
            </transition>
          </div>
        </div>
    ...
    <transition
      enter-active-class="transition-all duration-300 ease-out origin-top overflow-hidden"
      enter-from-class="transform scale-y-0 opacity-0 max-h-0"
      enter-to-class="transform scale-y-100 opacity-100 max-h-screen"
      leave-active-class="transition-all duration-200 ease-in origin-top overflow-hidden"
      leave-from-class="transform scale-y-100 opacity-100 max-h-screen"
      leave-to-class="transform scale-y-0 opacity-0 max-h-0"
    >
      <div v-if="mobileMenuOpen" class="md:hidden w-full bg-white border-t border-gray-100 shadow-inner" id="mobile-menu">
        <div class="px-2 pt-2 pb-3 space-y-1 sm:px-3">
          <ClientOnly>
            ...
            <template v-if="authStore.userRole === 'secretary'">
              ...
              <NuxtLink to="/trips" class="text-gray-600 hover:bg-gray-50 hover:text-gray-900 block px-3 py-2 rounded-md text-base font-medium" active-class="text-indigo-600 bg-indigo-50">Viajes</NuxtLink>
              ...
              <NuxtLink to="/packages" class="text-gray-600 hover:bg-gray-50 hover:text-gray-900 block px-3 py-2 rounded-md text-base font-medium" active-class="text-indigo-600 bg-indigo-50">Encomiendas</NuxtLink>
        ...
        <div class="pt-4 pb-3 border-t border-gray-200">
            <div class="flex items-center px-5">
                <div class="flex-shrink-0">
                    <div v-if="userInfo?.profileImageUrl" class="h-10 w-10 rounded-full overflow-hidden">
                      <img class="h-full w-full object-cover" :src="userInfo.profileImageUrl" alt="User profile">
                    </div>
                    <div v-else class="h-10 w-10 rounded-full bg-gradient-to-r from-blue-600 to-indigo-600 flex items-center justify-center text-white font-bold text-lg shadow-sm border border-white">
                      {{ userInitials }}
                    </div>
                </div>
                <div class="ml-3">
            ...
            <div class="mt-3 px-2 space-y-1">
                ...
                <a href="#" @click="handleLogout" class="block px-3 py-2 rounded-md text-base font-medium text-gray-500 hover:text-gray-900 hover:bg-gray-50">Cerrar sesión</a>
            </div>
        </div>
        </div>
    </transition>
