...
export function useTicketSale({ show, trip, selectedSeats, actionType, userId }: Args) {
  ...
  const [createdTicketsData, setCreatedTicketsData] = useState<Record<string, unknown>[]>([])
  ...
  const submit = async () => {
    ...
    try {
      ...
      if (clientSearch.clientType === 'new') {
        ...
        clientId = (clientRes as Record<string, unknown>).id as number | undefined
      }
      ...
      const seats = trip.seats_layout?.filter((s: Record<string, unknown>) =>
      ...
      const seatNumbers = createdTickets.map((t: Record<string, unknown>) => {
      ...
      if (error instanceof ApiError) {
        const detail = (error.data as Record<string, string>)?.detail || ''
        ...
        else setErrorMessage(detail || error.message)
      } else {
        setErrorMessage(error instanceof Error ? error.message : 'Error al crear los boletos')
      }
    } finally {
