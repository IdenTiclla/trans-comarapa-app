...
class CashRegisterRepository(BaseRepository[CashRegister]):
    ...
    def get_register_by_date(
        self, office_id: int, target_date: date
    ) -> Optional[CashRegister]:
        """Gets the cash register for a specific office and date."""
        return (
            self.db.query(CashRegister)
            .filter(
                CashRegister.office_id == office_id, CashRegister.date == target_date
            )
            .first()
        )
    ...
    def get_transactions(self, register_id: int) -> List[CashTransaction]:
        ...
        return (
            ...
            .filter(CashTransaction.cash_register_id == register_id)
    ...
    def get_closed_registers(
        self,
        office_id: Optional[int] = None,
        date_from: Optional[date] = None,
        date_to: Optional[date] = None,
    ) -> List[CashRegister]:
        """Gets closed cash registers filtered by office and date range."""
        ...
        return query.order_by(CashRegister.date.desc()).all()

    def count_transactions(self, register_id: int) -> int:
        """Counts transactions for a specific cash register."""
        return (
            self.db.query(CashTransaction)
            .filter(CashTransaction.cash_register_id == register_id)
            .count()
