...
async def get_ticket_stats(
    ...
    start_date, end_date, previous_start_date, previous_end_date = get_date_range(
        period
    )
    ...
    current_tickets = (
        ...
        .filter(
            ...
            TicketModel.state.in_(
                ["confirmed", "completed"]
            ),  # Solo tickets vendidos/completados
        )
        .first()
    )
...
async def get_trip_stats(
    ...
    current_trips = (
        db.query(func.count(TripModel.id).label("count"))
        .filter(
    ...
    return {"count": current_trips.count, "trend": round(trend, 2), "period": period}
...
async def get_tickets_summary_stats(
    ...
    total_revenue = (
        db.query(func.coalesce(func.sum(TicketModel.price), 0))
        .join(
            TicketStateHistoryModel,
            and_(
                TicketStateHistoryModel.ticket_id == TicketModel.id,
    ...
    for trip in upcoming_trips:
        ...
        occupied_seats = (
            ...
            .filter(
                ...
                ),  # Incluir reservas para contar asientos ocupados
            )
            .scalar()
        )
...
async def get_recent_sales(
    ...
    for package in recent_packages:
        ...
        full_package = (
            db.query(PackageModel).filter(PackageModel.id == package.id).first()
        )
...
async def get_sales_summary(
    ...
    current_total_amount = float(current_tickets.amount) + float(
        current_packages.amount
    )
    current_ticket_count = current_tickets.count
    current_package_count = current_packages.count

    # Calcular totales anteriores
    previous_total_amount = float(previous_tickets.amount) + float(
        previous_packages.amount
    )
...
async def get_reserved_ticket_stats(
    ...
    current_reserved_tickets = (
        db.query(func.count(TicketModel.id).label("count"))
        .filter(
            cast(TicketModel.created_at, Date) >= start_date,
            cast(TicketModel.created_at, Date) <= end_date,
            TicketModel.state == "pending",
    ...
    trend = calculate_trend(
        current_reserved_tickets.count, previous_reserved_tickets.count
    )
...
async def get_monthly_ticket_stats(
    months: int = Query(
        6, description="Número de meses para las estadísticas históricas"
    ),
    db: Session = Depends(get_db),
):
    """
    ...
    for row in monthly_data:
        ...
        data_dict[key] = {"count": row.count, "amount": float(row.amount)}
...
async def get_monthly_reservation_stats(
    months: int = Query(
        6, description="Número de meses para las estadísticas históricas de reservas"
    ),
    db: Session = Depends(get_db),
):
    """
    ...
    if len(result_data) >= 2:
        ...
        ):  # Añadido para tendencia negativa
            trend = -100.0
...
async def get_monthly_package_stats(
    months: int = Query(
        6, description="Número de meses para las estadísticas históricas de paquetes"
    ),
    db: Session = Depends(get_db),
):
    """
...
async def get_monthly_trip_stats(
    months: int = Query(
        6, description="Número de meses para las estadísticas históricas de viajes"
    ),
    db: Session = Depends(get_db),
):
    """
...
async def get_monthly_revenue_stats(
    months: int = Query(
        6, description="Número de meses para las estadísticas históricas de ingresos"
    ),
    db: Session = Depends(get_db),
):
    """
...
async def get_monthly_ticket_revenue_stats(
    months: int = Query(
        6,
        description="Número de meses para las estadísticas históricas de ingresos por boletos",
    ),
    db: Session = Depends(get_db),
):
    """
...
async def get_monthly_package_revenue_stats(
    months: int = Query(
        6,
        description="Número de meses para las estadísticas históricas de ingresos por paquetes",
    ),
    db: Session = Depends(get_db),
):
    """
...
async def get_monthly_cancelled_ticket_stats(
    months: int = Query(
        6,
        description="Número de meses para las estadísticas históricas de boletos cancelados",
    ),
    db: Session = Depends(get_db),
):
    """
...
async def get_monthly_completed_trip_stats(
    months: int = Query(
        6,
        description="Número de meses para las estadísticas históricas de viajes completados",
    ),
    db: Session = Depends(get_db),
):
    """
...
async def get_monthly_cancelled_trip_stats(
    months: int = Query(
        6,
        description="Número de meses para las estadísticas históricas de viajes cancelados",
    ),
    db: Session = Depends(get_db),
):
    """
...
async def get_monthly_client_feedback_stats(
    months: int = Query(
        6,
        description="Número de meses para las estadísticas históricas de feedback de clientes",
    ),
    db: Session = Depends(get_db),
):
    """
    ...
    monthly_data = (
        db.query(
            extract("year", TicketModel.created_at).label("year"),
            extract("month", TicketModel.created_at).label("month"),
            func.count(case((TicketModel.state != "cancelled", 1))).label(
                "successful_tickets"
            ),
...
async def get_monthly_registered_clients_stats(
    months: int = Query(
        6,
        description="Número de meses para las estadísticas históricas de clientes registrados",
    ),
    db: Session = Depends(get_db),
):
    """
...
async def get_monthly_delivered_packages_stats(
    months: int = Query(
        6,
        description="Número de meses para las estadísticas históricas de paquetes entregados",
    ),
    db: Session = Depends(get_db),
):
    """
...
async def get_cash_summary(
    ...
    """
    Obtener resumen financiero de caja del día
