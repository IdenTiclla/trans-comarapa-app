...
from schemas.office import OfficeResponse
...
class PackageBase(BaseModel):
    tracking_number: Optional[str] = Field(
        None,
        description="Número de encomienda único",
        json_schema_extra={"example": "ENC-003589"},
    )
    total_weight: Optional[float] = Field(
        None,
        description="Peso total del paquete en kg",
        json_schema_extra={"example": 5.2},
    )
    total_declared_value: Optional[float] = Field(
        None, description="Valor declarado total", json_schema_extra={"example": 500.0}
    )
    notes: Optional[str] = Field(
        None,
        description="Observaciones adicionales",
        json_schema_extra={"example": "Frágil - manejar con cuidado"},
    )
    status: str = Field(
        default="registered_at_office",
        description="Estado del paquete",
        json_schema_extra={"example": "registered_at_office"},
    )

    sender_id: int = Field(
        ..., description="ID del remitente", json_schema_extra={"example": 1}
    )
    recipient_id: int = Field(
        ..., description="ID del destinatario", json_schema_extra={"example": 2}
    )
    trip_id: Optional[int] = Field(
        None,
        description="ID del viaje (opcional, se asigna después)",
        json_schema_extra={"example": 3},
    )

    payment_status: str = Field(
        default="paid_on_send",
        description="Estado de pago",
        json_schema_extra={"example": "paid_on_send"},
    )
    payment_method: Optional[str] = Field(
        None, description="Método de pago", json_schema_extra={"example": "qr"}
    )

    origin_office_id: Optional[int] = Field(
        None, description="ID de la oficina de origen", json_schema_extra={"example": 1}
    )
    destination_office_id: Optional[int] = Field(
        None,
        description="ID de la oficina de destino",
...
class PackageCreate(PackageBase):
    items: List[PackageItemCreate] = Field(
        ..., description="Lista de items del paquete", min_length=1
    )
    secretary_id: int = Field(
        ...,
        description="ID de la secretaria que registra",
...
class PackageDeliverRequest(BaseModel):
    ...
    payment_method: str = Field(
        ..., description="Método de pago usado al entregar (cash/qr)"
    )
    changed_by_user_id: Optional[int] = Field(
        None, description="ID del usuario que realiza la entrega"
    )
...
class PackageResponse(PackageBase):
    ...
    secretary_id: int = Field(
        ..., description="ID de la secretaria", json_schema_extra={"example": 4}
    )
    delivered_by_secretary_id: Optional[int] = Field(
        None,
        description="ID de la secretaria que entregó",
        json_schema_extra={"example": 2},
    )
    ...
    items: List[PackageItemResponse] = Field(
        default=[], description="Items del paquete"
    )
    state_history: List[PackageStateHistorySchema] = Field(
        default=[], description="Historial de estados"
    )
    sender: Optional[Client] = None
    recipient: Optional[Client] = None
    trip: Optional[Trip] = None
    secretary: Optional[Secretary] = None
    origin_office: Optional[OfficeResponse] = None
    destination_office: Optional[OfficeResponse] = None
    delivered_by_secretary: Optional[Secretary] = None
...
class PackageSummary(BaseModel):
    ...
    payment_status: str
    payment_method: Optional[str] = None
    origin_office_id: Optional[int] = None
    destination_office_id: Optional[int] = None
    origin_office_name: Optional[str] = None
    destination_office_name: Optional[str] = None
    created_at: datetime
