...
| AUTH-04 | The system shall support password reset by Administrador. |
| AUTH-05 | Each user account shall be associated with a specific office (except Administrador, who has global access). |
...
| TRIP-04 | Only the Administrador can create, edit, and cancel trips. |

### 4.5 Ticket Sales

| ID | Requirement |
|----|-------------|
| TIX-01 | The Secretaria shall be able to search available trips by date and route. |
...
| TIX-03 | When a Secretaria selects a seat, the system shall temporarily lock it using Redis with a configurable TTL (e.g., 5 minutes) to prevent double-selling. |
...
| TIX-10 | The Secretaria shall be able to cancel a ticket (with appropriate permissions and audit trail). |

### 4.6 Parcel / Encomienda Management

| ID | Requirement |
|----|-------------|
| ENC-01 | The Secretaria shall be able to register a new encomienda with: sender name, sender phone, receiver name, receiver phone, origin office, destination office, description, weight/dimensions (optional), and declared value (optional). |
...
| ENC-06 | The destination office Secretaria shall be able to mark an encomienda as "delivered" and collect payment if it is "por cobrar." |
...
| CASH-02 | The Secretaria shall open the cash register at the start of their shift with an initial balance and close it at the end with a final count. |
...
| CASH-06 | The Administrador or authorized user shall be able to register a withdrawal when a fleet owner collects cash from an office. |
...
| RPT-04 | The Administrador shall be able to view reports for any office or across all offices. |
| RPT-05 | The Secretaria shall be able to view reports only for their own office. |
...
| USE-03 | The system shall provide clear error messages in Spanish. |
